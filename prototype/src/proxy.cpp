...
bool ProxyImpl::SetToMemcached(const char *key, size_t key_length,
  ...
  try {
    ...
    std::cout << "zhaoritian: "
              << "try to set " << key << std::endl;
    asio::ip::tcp::resolver resolver(io_context);
    asio::ip::tcp::resolver::results_type endpoint =
        resolver.resolve(std::string(ip), std::to_string(port));
    ...
    std::vector<unsigned char> int_buf_key_size =
        OppoProject::int_to_bytes(key_length);
    asio::write(socket,
                asio::buffer(int_buf_key_size, int_buf_key_size.size()));

    std::vector<unsigned char> int_buf_value_size =
        OppoProject::int_to_bytes(value_length);
    asio::write(socket,
                asio::buffer(int_buf_value_size, int_buf_value_size.size()));
    ...
    std::cout << "zhaoritian: "
              << "set " << key << " failed" << std::endl;
    exit(-1);
  }

  // std::cout << "set " << std::string(key) << " " << std::string(ip) << " " <<
  // port << " " << (MEMCACHED_SUCCESS==rc) << " " << value_length << std::endl;
  return true;
}
bool ProxyImpl::GetFromMemcached(const char *key, size_t key_length,
                                 char *value, size_t *value_length, int offset,
                                 int lenth, const char *ip, int port) {
  try {
    // std::unique_lock<std::mutex> lck(memcached_lock);
    std::cout << "GetFromMemcached"
              << " " << std::string(ip) << " " << port << std::endl;
    std::cout << "zhaoritian: "
              << "try to get " << key << std::endl;
    asio::ip::tcp::resolver resolver(io_context);
    asio::ip::tcp::resolver::results_type endpoint =
        resolver.resolve(std::string(ip), std::to_string(port));
    asio::ip::tcp::socket socket(io_context);
    ...
    std::vector<unsigned char> int_buf_offset =
        OppoProject::int_to_bytes(offset);
    ...
    asio::read(socket,
               asio::buffer(value_from_datanode, value_from_datanode.size()));
...
grpc::Status ProxyImpl::EncodeAndSetObject(
  ...
  OppoProject::EncodeType encode_type =
      (OppoProject::EncodeType)object_and_placement->encode_type();
  ...
  for (int i = 0; i < object_and_placement->datanodeip_size(); i++) {
    nodes_ip_and_port.push_back(
        std::make_pair(object_and_placement->datanodeip(i),
                       object_and_placement->datanodeport(i)));
  }
  auto encode_and_save = [this, big, key, value_size_bytes, k, m, real_l,
                          shard_size, tail_shard_size, stripe_ids,
                          nodes_ip_and_port, encode_type]() mutable {
    try {
      if (big == true) {
        ...
        // size_t len = asio::read(socket_data, asio::buffer(buf_key,
        // key.size()), error);
        ...
        if (flag) {
          // len = asio::read(socket_data, asio::buffer(v_buf,
          // value_size_bytes), error);
        ...
        char *buf = v_buf.data();
        auto send_to_datanode =
            [this](int j, int k, std::string shard_id, char **data,
                   char **coding, int x_shard_size,
                   std::pair<std::string, int> ip_and_port) {
              if (j < k) {
                SetToMemcached(shard_id.c_str(), shard_id.size(), data[j],
                               x_shard_size, ip_and_port.first.c_str(),
                               ip_and_port.second);
              } else {
                std::cout << j << " " << int(coding[j - k][0]) << std::endl;
                SetToMemcached(shard_id.c_str(), shard_id.size(), coding[j - k],
                               x_shard_size, ip_and_port.first.c_str(),
                               ip_and_port.second);
              }
            };
        for (int i = 0; i < int(stripe_ids.size()); i++) {
          ...
          std::vector<std::vector<char>> v_coding_area(
              m + real_l + 1, std::vector<char>(true_shard_size));
          for (int j = 0; j < k; j++) {
          ...
          for (int j = 0; j < send_num; j++) {
            ...
            std::pair<std::string, int> &ip_and_port =
                nodes_ip_and_port[i * send_num + j];
            senders.push_back(std::thread(send_to_datanode, j, k, shard_id,
                                          data, coding, true_shard_size,
                                          ip_and_port));
          }
...
grpc::Status ProxyImpl::WriteBufferAndEncode(
  ...
  if (buf_idx == -1) {
    ...
    auto encode_buf_and_save = [this, big, key, value_size_bytes, k, m, real_l,
                                shard_size, tail_shard_size, stripe_ids,
                                nodes_ip_and_port, encode_type]() mutable {
      try {
    ...
    auto write_buffer = [this, key, value_size_bytes, k, m, shard_size,
      ...
      if (flag) {
        len = socket_data.read_some(
            asio::buffer(&proxy_buf[buf_idx][buf_offset[buf_idx]],
                         value_size_bytes),
            error);
...
grpc::Status ProxyImpl::decodeAndGetObject(
  ...
  for (int i = 0; i < object_and_placement->datanodeip_size(); i++) {
    nodes_ip_and_port.push_back({object_and_placement->datanodeip(i),
                                 object_and_placement->datanodeport(i)});
  }

  auto decode_and_get = [this, big, key, k, m, real_l, shard_size,
                         tail_shard_size, value_size_bytes, clientip,
                         clientport, stripe_ids, nodes_ip_and_port,
                         encode_type]() mutable {
    if (big) {
      std::string value;
      for (int i = 0; i < int(stripe_ids.size()); i++) {
        ...
        int expect_block_number =
            (encode_type == Azure_LRC_1) ? (k + real_l - 1) : k;
        int all_expect_blocks =
            (encode_type == Azure_LRC_1) ? (k + m + real_l) : (k + m);
        all_expect_blocks = k;
        int send_num;
        if (encode_type == RS) {
          send_num = k + m;
        ...
        auto getFromNode = [this, k, shards_ptr, shards_idx_ptr, myLock_ptr,
                            cv_ptr](int expect_block_number, int stripe_id,
                                    int shard_idx, int x_shard_size,
                                    std::string ip, int port) {
          std::string shard_id = std::to_string(stripe_id * 1000 + shard_idx);
          std::vector<char> temp(x_shard_size);
          size_t temp_size;
          bool ret =
              GetFromMemcached(shard_id.c_str(), shard_id.size(), temp.data(),
                               &temp_size, 0, x_shard_size, ip.c_str(), port);
          ...
          if (!check_received_block(k, expect_block_number, shards_idx_ptr,
                                    shards_ptr->size())) {
        ...
        std::vector<std::vector<char>> v_data_area(
            k, std::vector<char>(true_shard_size));
        std::vector<std::vector<char>> v_coding_area(
            all_expect_blocks - k, std::vector<char>(true_shard_size));
        for (int j = 0; j < k; j++) {
        ...
        for (int j = 0; j < all_expect_blocks; j++) {
          ...
          read_memcached_treads.push_back(std::thread(
              getFromNode, expect_block_number, stripe_ids[i], j,
              true_shard_size, ip_and_port.first, ip_and_port.second));
        }
        for (int j = 0; j < all_expect_blocks; j++) {
        ...
        while (!check_received_block(k, expect_block_number, shards_idx_ptr,
                                     shards_ptr->size())) {
          cv_ptr->wait(lck);
        }
        ...
        for (int j = 0; j < all_expect_blocks; j++) {
          if (std::find(shards_idx_ptr->begin(), shards_idx_ptr->end(), j) ==
              shards_idx_ptr->end()) {
        ...
        if (encode_type == RS || encode_type == OPPO_LRC) {
          ...
          if (!decode(k, m, real_l, data, coding, erasures, true_shard_size,
                      encode_type, true)) {
...
grpc::Status
ProxyImpl::mainRepair(grpc::ServerContext *context,
  ...
  std::vector<std::pair<std::pair<std::string, int>, int>>
      inner_az_shards_to_read;
  for (int i = 0; i < mainRepairPlan->inner_az_help_shards_ip_size(); i++) {
    inner_az_shards_to_read.push_back(
        {{mainRepairPlan->inner_az_help_shards_ip(i),
          mainRepairPlan->inner_az_help_shards_port(i)},
  ...
  std::vector<std::pair<std::pair<std::string, int>, int>>
      new_locations_with_shard_idx;
  for (int i = 0; i < mainRepairPlan->new_location_ip_size(); i++) {
    new_locations_with_shard_idx.push_back(
        {{mainRepairPlan->new_location_ip(i),
          mainRepairPlan->new_location_port(i)},
  ...
  std::cout << std::endl;
  std::cout << "marker you" << std::endl;
  std::vector<int> chosen_shards;
  std::cout << "mainRepairPlan->chosen_shards_size(): "
            << mainRepairPlan->chosen_shards_size() << std::endl;
  ...
  if (one_shard_fail) {
    ...
    for (int i = 0; i < int(help_azs_id.size()); i++) {
      ...
      std::thread([&, socket_ptr]() {
        ...
        if (encode_type == Azure_LRC && all_failed_shards_idx[0] >= k &&
            all_failed_shards_idx[0] <= (k + g - 1)) {
          std::vector<unsigned char> int_buf(sizeof(int));
          asio::read(*socket_ptr, asio::buffer(int_buf, int_buf.size()));
    ...
    for (int i = 0; i < int(inner_az_shards_to_read.size()); i++) {
      readers_inner_az.push_back(std::thread([&, i]() {
        ...
        std::string shard_id = std::to_string(stripe_id * 1000 + shard_idx);
        size_t temp_size;
        GetFromMemcached(shard_id.c_str(), shard_id.size(), buf.data(),
                         &temp_size, 0, shard_size, ip.c_str(), port);
        repair_buffer_lock.lock();
    ...
    if (encode_type == Azure_LRC && all_failed_shards_idx[0] >= k &&
      ...
      std::cout << "****" << std::endl;
      int *result_matrix_ptr =
          jerasure_matrix_multiply(matrix1.data(), matrix2.data(),
                                   all_failed_shards_idx.size(), k, k, k, 8);
      for (int i = 0; i < all_failed_shards_idx.size(); i++) {
        for (int j = 0; j < k; j++) {
      ...
      memcpy(result_matrix.data(), result_matrix_ptr,
             result_matrix.size() * sizeof(int));
      ...
      for (auto &p : data_or_parity) {
        if (p.first == self_az_id) {
          ...
          for (int i = 0; i < all_failed_shards_idx.size(); i++) {
            ...
            jerasure_matrix_encode(count, 1, 8, encoding_matrix.data(), data,
                                   coding, shard_size);
            if (sum == 0) {
      ...
      for (int i = 0; i < all_failed_shards_idx.size(); i++) {
        ...
        jerasure_matrix_encode(count, 1, 8, new_matrix.data(), data, coding,
                               shard_size);
      }
    } else {
    ...
    std::cout << "new_node_ip： " << new_node_ip
              << "new_node_port: " << new_node_port << std::endl;
    std::string failed_shard_id =
        std::to_string(stripe_id * 1000 + failed_shard_idx);
    // SetToMemcached(failed_shard_id.c_str(), failed_shard_id.size(),
    // repaired_shard.data(), shard_size, new_node_ip.c_str(), new_node_port);
...
grpc::Status
ProxyImpl::helpRepair(grpc::ServerContext *context,
  ...
  for (int i = 0; i < helpRepairPlan->inner_az_help_shards_ip_size(); i++) {
    inner_az_shards_to_read.push_back(
        {{helpRepairPlan->inner_az_help_shards_ip(i),
          helpRepairPlan->inner_az_help_shards_port(i)},
  ...
  sort(all_failed_shards_idx.begin(), all_failed_shards_idx.end());
  sort(chosen_shards.begin(), chosen_shards.end());
  std::unordered_map<int, std::unordered_map<int, std::vector<char>>>
      data_or_parity;
  std::vector<std::thread> readers_inner_az;
  ...
  if (one_shard_fail) {
    ...
    asio::ip::tcp::resolver resolver(io_context);
    asio::ip::tcp::resolver::results_type endpoint =
        resolver.resolve(main_proxy_ip, std::to_string(main_proxy_port));
    asio::ip::tcp::socket socket(io_context);
    std::cout << "all_failed_shards_idx: " << all_failed_shards_idx.size()
              << " " << all_failed_shards_idx[0] << std::endl;
    std::cout << "main_proxy_ip " << main_proxy_ip
              << ", main_proxy_port: " << main_proxy_port << std::endl;
    std::cout << "self_az_id before: " << self_az_id << std::endl;
    asio::connect(socket, endpoint);
    std::vector<unsigned char> int_buf_self_az_id =
        OppoProject::int_to_bytes(self_az_id);
    std::cout << "self_az_id after: " << self_az_id << std::endl;
    asio::write(socket,
                asio::buffer(int_buf_self_az_id, int_buf_self_az_id.size()));
    std::cout << "first write" << std::endl;
    if (encode_type == Azure_LRC && all_failed_shards_idx[0] >= k &&
        all_failed_shards_idx[0] <= (k + g - 1)) {
      std::vector<unsigned char> int_buf_num_of_shards =
          OppoProject::int_to_bytes(all_failed_shards_idx.size());
      asio::write(socket, asio::buffer(int_buf_num_of_shards,
                                       int_buf_num_of_shards.size()));
      ...
      for (int i = 0; i < all_failed_shards_idx.size(); i++) {
        ...
        std::vector<unsigned char> int_buf_fail_idx =
            OppoProject::int_to_bytes(all_failed_shards_idx[i]);
        std::cout << "marker write" << std::endl;
        asio::write(socket,
                    asio::buffer(int_buf_fail_idx, int_buf_fail_idx.size()));
...
grpc::Status ProxyImpl::dataProxyUpdate(
  ...
  for (int i = 0; i < dataProxyPlan->receive_client_shard_idx_size(); i++) {
    ...
    data_nodes_ip_port.push_back(std::make_pair(
        dataProxyPlan->data_nodeip(i), dataProxyPlan->data_nodeport(i)));
  }
  for (int i = 0; i < dataProxyPlan->local_parity_idx_size(); i++) {
    localparity_idxes.push_back(dataProxyPlan->local_parity_idx(i));
    localparity_nodes_ip_port.push_back(
        std::make_pair(dataProxyPlan->local_parity_nodeip(i),
                       dataProxyPlan->local_parity_nodeport(i)));
  }
  // may need to call repair

  auto receive_update = [this, key, stripeid, update_op_id, data_idx_ranges,
                         data_nodes_ip_port, localparity_idxes,
                         localparity_nodes_ip_port, collector_proxyip,
                         collector_proxyprot]() mutable {
