...
void OppoProject::dfs(std::vector<int> temp,
                      std::shared_ptr<std::vector<std::vector<int>>> ans,
                      int cur, int n, int k) {
...
bool OppoProject::combine(std::shared_ptr<std::vector<std::vector<int>>> ans,
                          int n, int k) {
  std::vector<int> temp;
  dfs(temp, ans, 1, n, k);
  return true;
...
bool OppoProject::encode(int k, int m, int real_l, char **data_ptrs,
                         char **coding_ptrs, int blocksize,
...
bool OppoProject::decode(int k, int m, int real_l, char **data_ptrs,
                         char **coding_ptrs,
  ...
  if (encode_type == RS || encode_type == OPPO_LRC) {
    ...
    if (jerasure_matrix_decode(k, m + real_l, 8, matrix.data(), 0,
                               erasures->data(), data_ptrs, coding_ptrs,
                               blocksize) == -1) {
      ...
      for (int i = 0; i < k + m + real_l; i++) {
        if (std::find(erasures->begin(), erasures->end(), i) ==
            erasures->end()) {
      ...
      for (int i = 0; i < int(part_new_erasure->size()); i++) {
        for (int j = 0; j < int((*part_new_erasure)[i].size()); j++) {
          new_erasures[erasures->size() - 1 + j] =
              survival_index[(*part_new_erasure)[i][j] - 1];
        }

        if (jerasure_matrix_decode(k, m + real_l, 8, matrix.data(), 0,
                                   new_erasures.data(), data_ptrs, coding_ptrs,
                                   blocksize) != -1) {
          return true;
          break;
        }
...
bool OppoProject::check_received_block(
    int k, int expect_block_number,
    std::shared_ptr<std::vector<int>> shards_idx_ptr, int shards_ptr_size) {
  ...
  } else if (int(shards_idx_ptr->size()) == k) // azure_lrc防误杀
  {
    for (int i = 0; i < k; i++) {
      // 没找到
      if (std::find(shards_idx_ptr->begin(), shards_idx_ptr->end(), i) ==
          shards_idx_ptr->end()) {
        return false;
      }
    }
...
int OppoProject::check_decodable_azure_lrc(int k, int g, int l,
                                           std::vector<int> failed_block,
                                           std::vector<int> new_matrix) {
  // 数据块，全局校验块，局部校验块
  // 检查是否满足理论可解
  std::vector<int> survive_block;
  for (int i = 0; i < k + l + g; i++) {
    if (std::find(failed_block.begin(), failed_block.end(), i) ==
        failed_block.end()) {
