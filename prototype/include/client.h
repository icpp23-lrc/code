...
class Client {
public:
  Client(std::string ClientIP, int ClientPort, std::string CoordinatorIpPort)
      : m_coordinatorIpPort(CoordinatorIpPort), m_clientIPForGet(ClientIP),
        m_clientPortForGet(ClientPort),
        acceptor(io_context,
                 asio::ip::tcp::endpoint(
                     asio::ip::address::from_string(ClientIP.c_str()),
                     m_clientPortForGet)) {
  ...
  bool checkBias(double &node_storage_bias, double &node_network_bias,
    ...
    grpc::Status status =
        m_coordinator_ptr->checkBias(&context, request, &reply);
