...
// {
//     bool partial_decoding_helper(std::unordered_map<int, std::vector<char>>
//     all_origin, std::vector<int> failed_data_and_parity,
//     std::unordered_map<int, std::unordered_map<int, std::vector<char>>>
//     data_or_parity, int shard_size, int k, int g, int l)
//     {
...
//         {
//             if (std::find(failed_data_and_parity.begin(),
//             failed_data_and_parity.end(), i) == failed_data_and_parity.end())
//             {
...
//         {
//             if (check_decodable_azure_lrc(k, g, l, failed_data_and_parity,
//             matrix) != 1)
//             {
...
//         {
//             if (failed_data_and_parity[i] >= k && failed_data_and_parity[i] <
//             k + g + l)
//             {
...
//                 }
//                 std::cout << std::endl;
//                 jerasure_matrix_encode(count, 1, 8, new_matrix.data(), data,
//                 coding, shard_size); if (sum == 0)
//                 {
...
//             std::vector<int> new_matrix(1 * count, 1);
//             jerasure_matrix_encode(count, 1, 8, new_matrix.data(), data,
//             coding, shard_size); right[p] = temp;
//         }
//         std::vector<int> left;
//         std::sort(failed_data_and_parity.begin(),
//         failed_data_and_parity.end()); std::cout <<
//         "@@@@@@@@@@@@@@@@@@@@@@@@" << std::endl; for (auto &p :
//         failed_data_and_parity)
//         {
//             std::cout << p << " ";
//         }
//         std::cout << std::endl;
...
//         }
//         std::cout << "======================================================"
//         << std::endl; for (auto &p : left)
//         {
//             std::cout << p << " ";
//         }
//         std::cout << std::endl;
//         std::vector<int> invert(left.size());
//         jerasure_invert_matrix(left.data(), invert.data(),
//         failed_data_and_parity.size(), 8); for (auto &p : invert)
//         {
//             std::cout << p << " ";
//         }
//         std::cout << std::endl;
...
//         repaired_shards(failed_data_and_parity.size(),
//         std::vector<char>(shard_size)); int idx = 0; std::cout <<
//         "###############$$$$$$$$$$$$$$$$$$$$$" << std::endl; for (auto &p :
//         func_idx)
//         {
//             std::cout << p << " ";
...
//         }
//         std::cout << std::endl;
//         jerasure_matrix_encode(failed_data_and_parity.size(),
//         failed_data_and_parity.size(), 8, invert.data(), data, coding,
//         shard_size);
...
//                 << std::endl; std::cout <<
//                 "$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$"
//                 << std::endl; std::cout << newshard << std::endl;
//             }
//         }
//     }
// }
int main() {
  ...
  //     // std::cout << OppoProject::check_decodable_azure_lrc(k, g, l,
  //     failed_shard_idx1) << std::endl;
  //     // std::cout << "check_decodable_azure_lrc2" << std::endl;
  //     // std::vector<int> failed_shard_idx2 = {1, 2, 5, 8, 11, 15, 16, 19};
  //     // std::cout << OppoProject::check_decodable_azure_lrc(k, g, l,
  //     failed_shard_idx2) << std::endl;
  ...
  //     char **data = (char **)v_data.data();
  //     char **coding = (char **)v_coding.data();
  //     std::vector<std::vector<char>> coding_area(parity_blocks_number,
  //     std::vector<char>(shard_size));
  ...
  //     }
  //     OppoProject::encode(k, g, l, data, coding, shard_size,
  //     OppoProject::Azure_LRC_1);

  //     std::vector<int> failed_shard_idx = {2, 3, 6, 7, 14, 15, 18};
  //     std::unordered_map<int, std::unordered_map<int, std::vector<char>>>
  //     data_or_parity; int num_of_az = 3; int num_nodes_per_az = (k + g + l -
  //     failed_shard_idx.size()) / num_of_az; int idx = 0; for (int i = 0; i <
  //     num_of_az; i++)
  //     {
  //         for (int j = 0; j < num_nodes_per_az; j++)
  //         {
  //             while (failed_shard_idx.end() !=
  //             std::find(failed_shard_idx.begin(), failed_shard_idx.end(),
  //             idx))
  //             {
  //                 idx++;
  //             }
  ...
  //         }
  //     }
  //     OppoProject::partial_decoding_helper(all_origin, failed_shard_idx,
  //     data_or_parity, shard_size, k, g, l);
  return 0;
}