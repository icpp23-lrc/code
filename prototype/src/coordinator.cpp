...
grpc::Status CoordinatorImpl::uploadOriginKeyValue(
  ...
  */
  ...
  if (valuesizebytes > m_encode_parameter.small_file_upper) {
    ...
    static std::vector<int> temp_buf_rest(k,
                                          m_encode_parameter.blob_size_upper);
    ...
    if (buf_idx == -1) {
      ...
      generate_placement(m_Stripe_info[cur_stripe.Stripe_id].nodes,
                         cur_stripe.Stripe_id);
      for (int i = 0; i < int(m_Stripe_info[cur_stripe.Stripe_id].nodes.size());
           i++) {
        Nodeitem &node =
            m_Node_info[m_Stripe_info[cur_stripe.Stripe_id].nodes[i]];
        object_placement.add_datanodeip(node.Node_ip.c_str());
        object_placement.add_datanodeport(node.Node_port);
      }
      ...
      proxyIPPort->set_proxyport(selected_proxy_port + 1);
      if (status.ok()) {
        std::cout << "encode buffer success!" << std::endl;
      } else {
        std::cout << "encode buffer fail!" << std::endl;
        return grpc::Status::CANCELLED;
      }
      // TODO
      /*generate new stripe*/
    ...
    /*write buffer*/
    /*std::cout << "enter uploadOriginKeyValue smallwrite write buffer branch"
     * <<std::endl; */
...
grpc::Status CoordinatorImpl::requestRepair(
  ...
  for (auto stripe_id : failed_stripe_ids) {
    ...
    // 首先判断能不能解码：
    int survive_block = 0;
    for (int i = 0; i < stripe_info.k + stripe_info.g_m + stripe_info.real_l;
         i++) {
      if (std::find(failed_shard_idxs.begin(), failed_shard_idxs.end(), i) ==
          failed_shard_idxs.end()) {
        survive_block++;
      }
    }
    if (survive_block == stripe_info.k) {
      std::vector<int> matrix(
          (stripe_info.g_m + stripe_info.real_l) * stripe_info.k, 0);
      lrc_make_matrix(stripe_info.k, stripe_info.g_m, stripe_info.real_l,
                      matrix.data());
      if (check_decodable_azure_lrc(stripe_info.k, stripe_info.g_m,
                                    stripe_info.real_l, failed_shard_idxs,
                                    matrix) != 1) {
    ...
    for (int i = 0; i < int(failed_shard_idxs.size()); i++) {
      Nodeitem &node_info =
          m_Node_info[stripe_info.nodes[failed_shard_idxs[i]]];
      failed_shard_idxs_in_each_az[node_info.AZ_id].push_back(
          failed_shard_idxs[i]);
    }
    ...
    int block_bound_need_single_repair =
        (stripe_info.encodetype == Azure_LRC_1)
            ? (stripe_info.k + stripe_info.g_m + stripe_info.real_l)
            : (stripe_info.k + stripe_info.g_m);
...
void CoordinatorImpl::generate_repair_plan(
  ...
  if (one_shard) {
    ...
    // std::random_device rd;
    // std::mt19937 gen(rd());
    // std::uniform_int_distribution<unsigned int> dis(0,
    // m_AZ_info[main_az_id].nodes.size() - 1); do {
    //   int rand_idx = dis(gen);
    //   int node_id = m_AZ_info[main_az_id].nodes[rand_idx];
    //   auto lookup = std::find(stripe_info.nodes.begin(),
    //   stripe_info.nodes.end(), node_id); if (lookup ==
    //   stripe_info.nodes.end())
    //   {
    ...
    if (stripe_info.encodetype == RS) {
      ...
      for (int i = 0; i < int(shard_idx_for_repair.size()); i++) {
        ...
        if (node_info.AZ_id == main_az_id) {
          shards_to_read_for_main_az.push_back(
              {{node_info.Node_ip, node_info.Node_port}, shard_idx});
        } else {
      ...
      for (auto &az_and_shards_idx : azs_and_shards_idx) {
        ...
        std::vector<std::pair<std::pair<std::string, int>, int>>
            shards_to_read_for_help_az;
        for (auto &shard_idx : az_and_shards_idx.second) {
          Nodeitem &node_info = m_Node_info[stripe_info.nodes[shard_idx]];
          shards_to_read_for_help_az.push_back(
              {{node_info.Node_ip, node_info.Node_port}, shard_idx});
        }
      ...
      if (failed_shard_idx == (k + g + real_l) ||
          (failed_shard_idx >= k && failed_shard_idx <= (k + g - 1))) {
        ...
        if (failed_shard_idx != (k + g + real_l)) {
          ...
          temp3.push_back(
              {{node_info.Node_ip, node_info.Node_port}, k + g + real_l});
        }
      ...
      if (k + g + group_idx != failed_shard_idx) {
        ...
        temp5.push_back(
            {{node_info.Node_ip, node_info.Node_port}, k + g + group_idx});
      }
    ...
    for (int i = 0; i < int(stripe_info.nodes.size()); i++) {
      auto lookup =
          std::find(failed_shard_idxs.begin(), failed_shard_idxs.end(), i);
      // 排除局部校验块
      if (lookup == failed_shard_idxs.end()) {
        if (stripe_info.encodetype == RS ||
            stripe_info.encodetype == OPPO_LRC) {
          if (i < (k + g)) {
            Nodeitem &node_info = m_Node_info[stripe_info.nodes[i]];
    ...
    std::unordered_map<int, std::vector<int>>
        failed_shards_in_each_az_with_local;
    for (int i = 0; i < int(failed_shard_idxs.size()); i++) {
      Nodeitem &node_info =
          m_Node_info[stripe_info.nodes[failed_shard_idxs[i]]];
      failed_shards_in_each_az_with_local[node_info.AZ_id].push_back(
          failed_shard_idxs[i]);
    }
    // 给坏掉的节点寻找新节点
    ...
    if (m_encode_parameter.partial_decoding == false) {
      std::vector<std::pair<int, std::vector<int>>>
          sorted_live_shards_in_each_az;
      for (auto &p : live_shards_in_each_az) {
        sorted_live_shards_in_each_az.push_back({p.first, p.second});
      }
      // 按照az中存活块的数量排序
      std::sort(sorted_live_shards_in_each_az.begin(),
                sorted_live_shards_in_each_az.end(), cmp_num_live_shards);
      int count_shards = 0;
      int expect_shards_number =
          (stripe_info.encodetype == Azure_LRC_1) ? (k + 1) : (k);
      for (int i = 0; i < int(sorted_live_shards_in_each_az.size()); i++) {
        std::vector<std::pair<std::pair<std::string, int>, int>> temp;
        for (int j = 0; j < int(sorted_live_shards_in_each_az[i].second.size());
             j++) {
          ...
          if (shard_index != (k + g + real_l)) {
            ...
            temp.push_back(
                {{node_info.Node_ip, node_info.Node_port}, shard_index});
            count_shards++;
      ...
      int repair_limit_num =
          (stripe_info.encodetype == Azure_LRC_1) ? (k + g + real_l) : (k + g);
      for (auto &p : failed_shards_in_each_az_with_local) {
        for (auto &q : p.second) {
      ...
      for (int i = 0; i < int(failed_data_and_parity.size()); i++) {
        if (failed_data_and_parity[i] >= k &&
            failed_data_and_parity[i] < repair_limit_num) {
  ...
  if (encode_type == RS) {
    ...
    if (placement_type == Random) {
      ...
      for (int i = 0; i < k + g_m; i++) {
        ...
        } while (vis[node_idx] == true ||
                 num_chosen_nodes_per_az[m_Node_info[node_idx].AZ_id] == k);
    ...
    if (placement_type == Random) {
      ...
      int start_idx = 0;
      int sita = g_m / b;
      int num_nodes = tail_group > 0 ? (k + g_m + full_group_l + 1 + 1)
                                     : (k + g_m + full_group_l + 1);
        ...
        while (left_data_shard > 0) {
          if (left_data_shard >= sita * b) {
            ...
            for (int i = 0; i < sita; i++) {
              az.cur_node = az.cur_node % az.nodes.size();
              stripe_nodes[k + g_m + start_idx / b + i] =
                  az.nodes[az.cur_node++];
              m_Node_info[az.nodes[az.cur_node - 1]].stripes.insert(stripe_id);
            }
            start_idx += (sita * b);
            ...
            if (left_data_shard > 0) {
              for (int i = 0; i < left_data_shard; i++) {
                az.cur_node = az.cur_node % az.nodes.size();
                stripe_nodes[start_idx + i] = az.nodes[az.cur_node++];
                m_Node_info[az.nodes[az.cur_node - 1]].stripes.insert(
                    stripe_id);
              }
              az.cur_node = az.cur_node % az.nodes.size();
    ...
    if (placement_type == Random) {
      ...
      if (tail_group > 0) {
        ...
        help[az_idx].second++;
               ...
               placement_type == Best_Best_Placement ||
...
// update
// Cross AZ aware
grpc::Status CoordinatorImpl::updateGetLocation(
    ::grpc::ServerContext *context,
  ...
  try {
    auto updated_stripe_shards =
        split_update_length(key, update_offset_infile, update_length);
    ...
    if (temp_stripe.encodetype == Azure_LRC_1 ||
        temp_stripe.encodetype == OPPO_LRC) {
      for (int i = temp_stripe.k + temp_stripe.g_m;
           i < temp_stripe.k + temp_stripe.g_m + temp_stripe.real_l; i++) {
        Nodeitem &tempnode = m_Node_info[temp_stripe.nodes[i]];
        int AZid = tempnode.AZ_id;
    ...
    if (temp_stripe.encodetype == Azure_LRC_1) { // other placement to be done
      std::map<int, std::vector<ShardidxRange>>::iterator max_num_idx_iter =
          AZ_updated_idxrange.begin();
      for (auto it = AZ_updated_idxrange.begin();
           it != AZ_updated_idxrange.end(); it++) {
        if ((it->second).size() > (max_num_idx_iter->second).size())
          max_num_idx_iter = it;
      }

      auto max_num_global_iter = AZ_global_parity_idx.begin();
      for (auto it = AZ_global_parity_idx.begin();
           it != AZ_global_parity_idx.end(); it++)
      ...
      for (auto it = AZ_global_parity_idx.begin();
        ...
        int temp_p_num = AZ_global_parity_idx[temp_AZid].size() +
                         AZ_local_parity_idx[temp_p_num].size();
        int max_p_num = AZ_global_parity_idx[max_AZid].size() +
                        AZ_local_parity_idx[max_p_num].size();
    ...
    if (temp_stripe.encodetype == Azure_LRC_1) {
      ...
      for (auto const &t_item : AZ_updated_idxrange) {
        ...
        for (int i = 0; i < t_idxranges.size(); i++) {
          ...
          collector_notice.add_receive_proxy_shard_offset(
              t_idxranges[i].offset_in_shard);
          collector_notice.add_receive_proxy_shard_length(
              t_idxranges[i].range_length);
        }
      }
...
CoordinatorImpl::split_update_length(std::string key, int update_offset_infile,
                                     int update_length) {
  ...
  if (!object.big_object) {
    ...
    updated_stripe_shards[stripe.Stripe_id].push_back(
        ShardidxRange(object.shard_idx, update_offset_inshard, update_length));
  } else {
    ...
    int first_offset_in_shard =
        desend_offset - first_update_idx * first_updated_stripe.shard_size;
    ...
    ShardidxRange first_idx_range(first_update_idx, first_offset_in_shard,
                                  first_shard_update_len);
    updated_stripe_shards[first_updated_stripe.Stripe_id].push_back(
        first_idx_range);
    if (desend_len > 0 && first_update_idx <= first_updated_stripe.k -
                                                  2) { // other idx of 1st stipe
      ...
      for (; tt_idx < first_updated_stripe.k && desend_len > 0; tt_idx++) {
        if (desend_len > first_updated_stripe.shard_size) {
          ...
          updated_stripe_shards[first_updated_stripe.Stripe_id].push_back(
              ShardidxRange(tt_idx, temp_offset, temp_len));
          desend_len -= temp_len;
        } else {
          int temp_offset = 0;
    ...
    if (desend_len > 0) { // more than 1 stripe
      ...
      for (; i < stripe_ids.size() && desend_len > 0; i++) {
        StripeItem &temp_stripe = m_Stripe_info[stripe_ids[i]];
        for (int tt_idx = 0; tt_idx < temp_stripe.k && desend_len > 0;
             tt_idx++) { // each shard
          if (desend_len > temp_stripe.shard_size) {
            int temp_offset = 0;
            int temp_len = temp_stripe.shard_size;
            updated_stripe_shards[temp_stripe.Stripe_id].push_back(
                ShardidxRange(tt_idx, temp_offset, temp_len));
            desend_len -= temp_len;
          } else {
            int temp_offset = 0;
